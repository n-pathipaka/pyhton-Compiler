...
class Node:
    ...
    def __init__(self, github, name = None, color='white', dot=None, neighbors=[], npos_reg=set()):
        self.name = name
        self.color = color
...
class Graph:
    ...
    def get_node_register(self, name):
        for node in self.nodes:
            if node.name == name:
                return node.reg_alloc

    def create_nodes(self, nodes_list, color=None):
        for node in nodes_list:
            self.nodes.append(Node(self.github, node, color, self.dot, [], set()))
        
        self.edges = {}

    def add_node(self, name, color=None):
        self.nodes.append(Node(self.github, name, color, self.dot, [], set()) )
    ...
    def draw_graph(self):
        ...
        self.dot.render('interference_graph.gv', view=False)
